...
import com.xd.pre.flowable.common.cmd.DeployModelCmd;
import com.xd.pre.flowable.common.cmd.SaveModelEditorCmd;
import com.xd.pre.flowable.vo.ModelRequest;
import com.xd.pre.flowable.vo.ModelResponse;
import com.xd.pre.flowable.vo.query.ModelQueryVo;
import com.xd.pre.flowable.wapper.ModelListWrapper;
...
public class ModelController extends BaseFlowableController {
    ...
    @PreAuthorize("hasAuthority('flowable:model:list')")
    @GetMapping(value = "/list")
    public R list(ModelQueryVo modelQueryVo) {
    ...
    public R queryById(@RequestParam String id) throws UnsupportedEncodingException {
    ...
    @SysOperaLog(descrption = "新增流程模型")
    @PreAuthorize("hasAuthority('flowable:model:save')")
    @PostMapping(value = "/save")
    @Transactional(rollbackFor = Exception.class)
    public R save(@RequestBody ModelRequest modelRequest) {
    ...
    @SysOperaLog(descrption = "修改流程模型")
    @PreAuthorize("hasAuthority('flowable:model:update')")
    @PutMapping(value = "/update")
    @Transactional(rollbackFor = Exception.class)
    public R update(@RequestBody ModelRequest modelRequest) {
    ...
    @SysOperaLog(descrption = "删除流程模型")
    @PreAuthorize("hasAuthority('flowable:model:delete')")
    @DeleteMapping(value = "/delete")
    @Transactional(rollbackFor = Exception.class)
    public R delete(@RequestParam String ids, @RequestParam(required = false) boolean cascade) {
    ...
    @SysOperaLog(descrption = "保存流程模型")
    @PreAuthorize("hasAuthority('flowable:model:saveModelEditor')")
    @PutMapping(value = "/saveModelEditor")
    @Transactional(rollbackFor = Exception.class)
    public R saveModelEditor(@RequestBody ModelRequest modelRequest) {
        managementService.executeCommand(new SaveModelEditorCmd(modelRequest.getId(), modelRequest.getEditor()));
        return R.ok();
    }

    @SysOperaLog(descrption = "部署流程模型")
    @PreAuthorize("hasAuthority('flowable:model:deploy')")
    @PostMapping(value = "/deploy")
    @Transactional(rollbackFor = Exception.class)
    public R deployModel(@RequestBody ModelRequest modelRequest) {
        managementService.executeCommand(new DeployModelCmd(modelRequest.getId()));
        return R.ok();
    }

    @SysOperaLog(descrption = "导入流程模型")
    @PreAuthorize("hasAuthority('flowable:model:import')")
    @PostMapping(value = "/import")
    @Transactional(rollbackFor = Exception.class)
    public R doImport(@RequestParam(required = false) String tenantId, HttpServletRequest request) throws IOException {
